...
### Added
- **F627 llm + service-proxy → core/infra/ (Sprint 350 — F596 cluster 평탄 구조 2회차 재현)** (PR #749 + auto-SPEC PR #788, S344, 2026-05-09 21:05 KST) — autopilot Match **100%** ~7분 27초 (시동~MERGED). git mv 2 + `core/infra/types.ts` re-export 5 symbols (LLMService + LLMTask + LLMTaskResult + ServiceProxy + ServiceProxyResult) + 13 callers path 갱신 (modules/portal 2 + core/sr 2 + core/agent 2 + core/spec 1 + core/harness 1 + tests 4 + routes/proxy 1). **자율 보정**: service-proxy env depth fix + conflict-detector hidden ref 발견/수정 (autopilot Plan 외 fs 실측 보정, S280/S282 패턴 26회차). services/ 루트 6→**4 files** (merge-queue/conflict-detector/pii-masker/pr-pipeline 잔존), core/infra/ source 4→**6 files**. typecheck PASS, 2399/2401 tests GREEN. **Stale F_ITEMS 패턴 10회차 재현** (rules/development-workflow.md 9→10 누적, signal F635 + .sprint-context Sprint 370 stale → 표준 보정 정상). **37 세션 연속 성공** (S306~S344, F560~F627). **다음 사이클 후보** (out of scope): pii-masker → core/infra/ 또는 security 신설 / conflict-detector → core/spec/services/ 도메인 결정 / pr-pipeline → core/agent/services/ / merge-queue 도메인 결정 / routes/proxy.ts 별 closure / Phase 47 GAP-3 27 stale proposals / AI Foundry W19 BeSir 5/15 D-6 / F638 본 통합 (F637 PoC scope 분리 → 5 packages 일괄)
